...
for i=1:2 % first loop fills in with the deeper stations
    inds2=data.Station==31; % pick a station
    gridded(floor(data.Depthm(inds2)),i)=data.Beamattenuationm1(inds2)'; % fill in the contour
end
...
gridded(99:200,1)=data.Beamattenuationm1(data.Station==33 & data.Depthm==99); % fill in a depth issue
gridded(190:220,2)=data.Beamattenuationm1(data.Station==32 & data.Depthm==189); % fill in a depth issue
...
figure(3)
contourf(xrange,yrange,gridded,'LineColor','none')
hold on
plot(data.Longitude,data.Depthm,'ko','MarkerFaceColor','k','MarkerSize',1)
a=area(coast_x(ind2),bathy','FaceColor','none','EdgeColor','none');
a(2).FaceColor=[160/255 160/255 160/255];
...
set(get(h,'label'),'string','Beam attenuation/m','FontSize',12);
ylabel('Depth/m')
% set(gca,'YTickLabels',[]);
xlabel(['Longitude/' char(176)' 'E'])
% title('Hecata Bank')
hold off
saveas(gcf,'Hecata_trans.svg')
...
for i=1:2 % first loop fills in with the deeper stations
    inds2=data.Station==31; % pick a station
    gridded(floor(data.Depthm(inds2)),i)=data.Chlorophyll(inds2)'; % fill in the contour
end
...
gridded(99:200,1)=data.Chlorophyll(data.Station==33 & data.Depthm==99); % fill in a depth issue
gridded(190:220,2)=data.Chlorophyll(data.Station==32 & data.Depthm==189); % fill in a depth issue
...
figure(4)
contourf(xrange,yrange,gridded,[0.1:1:cmax],'LineColor','none')
hold on
plot(data.Longitude,data.Depthm,'ko','MarkerFaceColor','k','MarkerSize',1)
a=area(coast_x(ind2),bathy','FaceColor','none','EdgeColor','none');
a(2).FaceColor=[160/255 160/255 160/255];
...
set(get(h,'label'),'string','Chl-a fluorescence/mg m^{-3}','FontSize',12);
ylabel('Depth/m')
% set(gca,'YTickLabels',[]);
xlabel(['Longitude/' char(176)' 'E'])
% title('Hecata Bank')
hold off
saveas(gcf,'Hecata_chla.svg')
